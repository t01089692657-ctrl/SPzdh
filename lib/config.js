...
function loadConfig() {
  ...
  const config = {
    ...
    commands: {
      ...
      // Codex 以绕过沙箱方式执行用户素材+提示，存在提示注入即在服务器上执行代码的风险。
      // 默认关闭；仅在你信任所有可登录员工、且需要 Codex 深度分析时，由管理员显式开启。
      codexEnabled: merged.commands?.codexEnabled === true
    },
    session: {
    ...
    limits: {
      // 用正数校验兜底：config 里误写成 "large"/"120mb" 等非数字时回退默认值，
      // 否则 NaN 会让 body 大小上限彻底失效（size > NaN 恒为 false），内存无保护
      bodyLimitMb: positiveNumber(merged.limits?.bodyLimitMb, 120),
      jobHistory: positiveNumber(merged.limits?.jobHistory, 200)
    }
  };
