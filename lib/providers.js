...
// 只把 command（可能是含空格的绝对路径）加引号；参数不在此处消毒——用户自由文本(prompt)
// 已在调用点用 sanitizeCmdArg 处理，其余参数（flag/文件路径/白名单字段）保持原样。
function runCli(command, args, options = {}) {
  if (IS_WINDOWS) {
    const quotedCommand = /[\s&()[\]{}^=;!'+,`~]/.test(command) ? `"${command}"` : command;
    return runCommand("cmd.exe", ["/d", "/s", "/c", quotedCommand, ...args], options);
  }
...
function resolveLocalOutput(url, options) {
  ...
  const relative = decodeURIComponent(pathname.replace(/^\/outputs\//, ""));
