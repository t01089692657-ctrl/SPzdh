...
- 密码用 scrypt 加盐哈希存储；登录有失败次数限制（10 次/10 分钟）；初始管理员密码首启随机生成并打印在控制台/`data/初始管理员密码.txt`
- 所有业务接口都要求登录；成品视频 `/outputs/` 需登录且**按属主隔离**（员工只能看自己的结果，管理员可看全部）
- 用户可控文本进入命令行前已消毒（防命令注入）；剪辑引用外链做了 SSRF 拦截（内网/本机地址不可下载）
- 平台设计为**公司局域网内使用**。如需公网访问，请套一层 HTTPS 反向代理
  （如 nginx + 证书），不要把端口直接暴露到公网
- API Key 只存在服务器的 config.json / 环境变量里，不会下发给浏览器
- **Codex 自动分析默认关闭**：它会以绕过沙箱的方式在服务器上执行分析，恶意提示词可能导致服务器被执行任意代码。
  只有当你信任所有可登录员工、且确实需要 Codex 深度分析时，再由管理员在 `config.json` 设置
  `commands.codexEnabled: true` 开启；建议只在隔离/可信服务器上启用。

## 已知限制与提示

- **交叉淡化转场**：随包 ffmpeg 是稳定静态版（无 xfade 滤镜），选“交叉淡入淡出”会自动降级为黑场淡入淡出。
  想要真正的交叉淡化，运行 `deps/get-ffmpeg` 或自行装 ffmpeg 4.3+ 到系统 PATH，平台会自动优先用新版。
- **即梦 CLI 与 WSL**：Windows 上即梦命令行常装在 WSL 里，平台默认经 `bash.exe` 调用（`commands.dreaminaViaWsl`）。
  如果你的机器装的是 Git-Bash 而非 WSL，或即梦直接装在 Windows 原生环境，请把 `dreaminaViaWsl` 设为 `false`，
  并把 `commands.dreamina` 指向真实可执行文件。
- **中文 Windows 的 CLI 输出**：个别第三方 CLI 在中文 Windows 上按 GBK 输出，日志里的中文可能显示为乱码，
  但视频链接/ID 等 ASCII 内容不受影响，不影响生成与剪辑。
